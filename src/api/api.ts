...
export const getMyOrders = () => {
    ...
    return useQuery({
        queryKey: ['orders', id],
        queryFn: async () => {
            const { data, error } = await supabase.from('order').select('*').order('create_at', { ascending: false }).eq('user', id);
            if (error) {
...
export const createOrder = () => {
    ...
    return useMutation({
        async mutationFn({ totalPrice }: { totalPrice: number }) {
            ...
            if (error) {
                throw new Error('An error occurred while creating order: ' + error?.message);
            }
            return data;
        },
        async onSuccess() {
...
export const createOrderItem = () => {
    return useMutation({
        async mutationFn(insertData: { orderId: number, productId: number, quantity: number }[]) {
            const { data, error } = await supabase.from("order_item").insert(insertData.map(({
